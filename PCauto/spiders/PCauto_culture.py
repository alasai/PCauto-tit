...
from PCauto.items import PCautoCultureItem

class PCautoCultureSpider(RedisSpider):
    name = 'PCauto_culture'
    index_page = 'http://www.pcauto.com.cn/wenhua/'

    pipeline = set([pipelines.CulturePipeline, ])
    ...
    def get_nav(self,response):
        ...
        for nav in nav_list[1:-1]:
            href = nav.xpath('./a/@href')[0]
            yield Request(href, dont_filter=True, callback=self.get_page)
            yield Request(href, callback=self.get_url)
        youji_url = nav_list[-1].xpath('./a/@href')[0]
        yield Request(youji_url, dont_filter=True, callback=self.get_youji)
        yield Request(youji_url, callback=self.get_url)


    def get_youji(self,response):
        model = etree.HTML(response.body_as_unicode())
        youji_list = model.xpath('//ul[@class="ulPic"]/li')
        for youji in youji_list:
            href = youji.xpath('./a/@href')[0]
            yield Request(href, callback=self.get_url)

        next_page = model.xpath('//div[@id="pcauto_page"]/a[@class="next"]')
        if next_page:
            next_page_url = next_page[0].xpath('./@href')[0]
            yield Request(next_page_url, dont_filter=True, callback=self.get_youji)
