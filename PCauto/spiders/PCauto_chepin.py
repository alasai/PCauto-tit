...
from PCauto.items import PCautoChepinItem

class PCautoChepinSpider(RedisSpider):
    name = 'PCauto_chepin'
    index_page = 'http://www.pcauto.com.cn/drivers/yongpin/'

    pipeline = set([pipelines.ChepinPipeline, ])
    ...
    def get_nav(self,response):
        ...
        for nav in nav_list[3:]:
            href = nav.xpath('./a/@href')[0]
            yield Request(href, dont_filter=True, callback=self.get_column)
            yield Request(href, callback=self.get_url)

    def get_column(self,response):
        model = etree.HTML(response.body_as_unicode())
        pcdg_url = model.xpath('//div[@class="box box-2 box-4 mt25"]//h2[@class="mark"]/a/@href')[0]
        yield Request(pcdg_url, dont_filter=True, callback=self.get_page)
        yield Request(pcdg_url, callback=self.get_url)
        csyw_url = model.xpath('//div[@class="box box-2 box-4 mt17"]//h2[@class="mark"]/a/@href')[0]
